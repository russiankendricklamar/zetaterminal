...
\[
D_{\text{Macaulay}} = \frac{\sum_{i=1}^{n} t_i \times \frac{CF_i}{(1 + r)^{t_i}}}{P_{\text{dirty}}}
\]
...
\[
C = \frac{1}{P_{\text{dirty}}} \times \frac{1}{(1 + r)^2} \sum_{i=1}^{n} \frac{CF_i \times t_i \times (t_i + 1)}{(1 + r)^{t_i}}
\]
...
**30/360 (US):**
\[
\text{Year Fraction} = \frac{360 \times (Y_2 - Y_1) + 30 \times (M_2 - M_1) + (D_2 - D_1)}{360}
\]
...
\[
V_{\text{fixed}} = N \times \sum_{i=1}^{n} \frac{C \times \tau_i}{(1 + r_i)^{t_i}}
\]
...
\[
V_{\text{floating}} = N \times \left( \frac{1}{(1 + r_1)^{t_1}} - \frac{1}{(1 + r_n)^{t_n}} \right)
\]
...
\[
D = -\frac{1}{V} \times \frac{\partial V}{\partial r}
\]
...
\[
F = (S - I) \times e^{r \times T} + \text{AI}_{\text{delivery}}
\]
