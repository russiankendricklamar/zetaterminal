...
Zeta Terminal реализует **decoupled client-server архитектуру** с чётким разделением ответственности:
...
| SQLAlchemy | 2.0.* | Async ORM (Neon PostgreSQL via asyncpg) |
...
5. Репозиторий обращается к Neon PostgreSQL (опционально)
