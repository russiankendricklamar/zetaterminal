...
import numpy as np
from numpy.linalg import LinAlgError, eig, lstsq
...
class SpectralRegimeAnalyzer:
    ...
    def __init__(
        ...
        # Результаты анализа
        self.poles: np.ndarray | None = None
        self.residues: np.ndarray | None = None
        self.regime_poles: np.ndarray | None = None
        ...
        self.H_spectrum: np.ndarray | None = None
        self.amplitude_spectrum: np.ndarray | None = None
        self.log_amplitude_spectrum: np.ndarray | None = None
        self.phase_spectrum: np.ndarray | None = None
        self.is_minimum_phase: bool = False

        # Динамический анализ
        self.regime_signal: np.ndarray | None = None
        self.regime_energies: np.ndarray | None = None
    ...
    def dynamic_regime_analysis(self, y: np.ndarray) -> None:
        ...
        for t_idx in range(n_points):
            ...
            probs = self.regime_energies[t_idx, :] / total_energy if total_energy > 0 else np.ones(self.K) / self.K
    ...
    def reconstruction_error(self) -> tuple[float, float]:
        ...
        pct_explained = (1 - rmse ** 2 / var_true) * 100 if var_true > 0 else 100.0
        ...
        for M in n_poles_range:
            try:
                ...
                if criterion.lower() == 'aic':
                    ...
                    correction = 2 * k * (k + 1) / (n - k - 1) if n > k + 1 else 0
    ...
    def get_summary(self) -> dict:
        ...
        _is_min_phase, min_phase_info = self.minimum_phase_check()
