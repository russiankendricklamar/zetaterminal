...
import numpy as np
import pandas as pd
from numpy.linalg import LinAlgError, det, inv
...
class MultivariateHMMRegimeAnalyzer:
    ...
    def predict_states(self, y: np.ndarray | None = None) -> np.ndarray:
        """
    ...
    def get_regime_statistics(self) -> list[dict[str, Any]]:
        ...
        for k in range(M):
            ...
            duration_days = -1.0 / np.log(max(persistence, 1e-300)) if persistence < 1.0 else np.inf
    ...
    def print_summary(self):
        ...
        for k in range(self.n_regimes):
            ...
            for _i, (_name, _mean_val) in enumerate(zip(self.asset_names, stats['mean'], strict=False)):
                pass
