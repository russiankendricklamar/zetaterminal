...
def fetch_zcyc_from_moex(date: str | None = None) -> dict:
    """
    ...
    try:
        ...
        for row in zcyc_data:
            ...
            try:
                ...
                if term_years is not None and yield_value is not None and \
                   term_years == term_years and yield_value == yield_value:  # Проверка на NaN
                    points.append({
        ...
        return {
            'status': 'error',
            'error': f'Ошибка подключения к MOEX API: {e!s}',
        ...
        return {
            'status': 'error',
            'error': f'Ошибка обработки данных: {e!s}',
            'date': date or datetime.now().strftime("%Y-%m-%d"),
            'data': [],
            'count': 0,
            'min_term': 0.0,
...
def build_zcyc_from_bonds(date: str | None = None) -> dict:
    """
    ...
    try:
        ...
        return {
            'status': 'error',
            'error': f'Ошибка построения кривой: {e!s}',
            'date': date or datetime.now().strftime("%Y-%m-%d"),
            'data': [],
            'count': 0,
            'min_term': 0.0,
...
def interpolate_zcyc_rate(
    ...
    for point in zcyc_data:
        if point['term'] <= term and (lower is None or point['term'] > lower['term']):
            lower = point
        if point['term'] >= term and (upper is None or point['term'] < upper['term']):
            upper = point
...
def get_yearyields(date: str | None = None, engine: str = "stock") -> pd.DataFrame:
    """
...
def get_yearyields_dates(date: str | None = None, engine: str = "stock") -> pd.DataFrame:
    """
