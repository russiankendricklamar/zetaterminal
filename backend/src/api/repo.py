...
async def analyze_repo(request: Request, file: UploadFile = File(...)):
    ...
    df = await asyncio.to_thread(
        lambda: pd.read_csv(io.BytesIO(content)) if ext == "csv" else pd.read_excel(io.BytesIO(content))
    )
    if df.empty:
        raise HTTPException(status_code=400, detail="Uploaded file is empty")

    result = await asyncio.to_thread(run_full_pipeline, df)
    return result
