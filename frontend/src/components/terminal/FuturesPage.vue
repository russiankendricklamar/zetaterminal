<template>
  <div class="page-container custom-scrollbar">
      ...
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <p class="section-subtitle font-mono">
            КОНТРАКТЫ MOEX FORTS, ОИ И ОБЪЁМ
          </p>
        </div>
      </div>
      <div class="tab-group">
    ...
    >
      <div class="w-3 h-3 border-2 border-cyan-500 border-t-transparent rounded-full animate-spin" />
    ...
    <div class="flex-1 flex flex-col gap-6">
      ...
      >
        ...
        <div
          v-if="futuresContracts.length > 0"
            ...
            <tbody class="text-sm font-mono text-gray-300">
              ...
              >
                ...
                <td class="p-4 text-right font-bold text-white">
                  {{ c.last != null ? c.last.toLocaleString() : '—' }}
                </td>
                <td class="p-4 text-right">
                  <span
                    v-if="c.change_pct != null"
                    :class="c.change_pct >= 0 ? 'text-emerald-400' : 'text-rose-400'"
                ...
                <td class="p-4 text-right text-gray-400 text-xs">
                  {{ c.volume != null ? formatVolume(c.volume) : '—' }}
                </td>
                <td class="p-4 text-right text-cyan-400 text-xs">
                  {{ c.oi != null ? formatVolume(c.oi) : '—' }}
                </td>
              </tr>
            </tbody>
          </table>
        ...
        <div
          v-if="!loading && futuresContracts.length === 0 && !loadError"
      ...
      <div
        v-else-if="section === 'FWCV'"
        class="flex flex-col h-full gap-6"
      >
        <h3 class="text-lg font-bold text-white">
          Открытый интерес — детали
        </h3>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div
            v-for="ticker in oiTickers"
            :key="ticker"
            class="p-4 rounded-xl bg-white/5 border border-white/5 text-center cursor-pointer hover:bg-white/10 transition-colors"
            @click="loadOI(ticker)"
          >
          ...
          <table class="w-full text-left border-collapse">
            ...
            <tbody class="text-sm font-mono text-gray-300">
              ...
              >
                ...
                <td class="p-4 text-right text-cyan-400">
                  {{ formatVolume(item.oi) }}
                </td>
