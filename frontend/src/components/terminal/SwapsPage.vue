<template>
  <div class="page-container custom-scrollbar">
      ...
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <p class="section-subtitle font-mono">
            ЦЕНООБРАЗОВАНИЕ, ОЦЕНКА И КРИВЫЕ
          </p>
        </div>
      </div>
      <div class="tab-group">
    ...
    <div class="flex-1 flex flex-col gap-6 p-6">
      ...
      >
        <div class="space-y-6">
          <div class="p-6 rounded-2xl bg-white/5 border border-white/5">
            ...
            <div class="space-y-4">
              <div>
                <label class="text-xs text-gray-500 block mb-1">Notional ($M)</label><input
                  v-model.number="notional"
                  type="number"
                  class="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-white font-mono text-sm"
                >
              </div>
              <div>
                <label class="text-xs text-gray-500 block mb-1">Fixed Rate (%)</label><input
              ...
              <div>
                <label class="text-xs text-gray-500 block mb-1">Tenor (Years)</label><input
            ...
            <button
              class="w-full mt-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-bold"
              @click="calculateNpv"
            >
              Calculate NPV
            </button>
              ...
              <tbody class="text-sm font-mono text-gray-300">
                ...
                >
                  ...
                  <td class="p-3 text-right text-rose-400">
                    -${{ cf.fixedLeg.toLocaleString() }}
                  </td>
                  <td class="p-3 text-right text-emerald-400">
                    +${{ cf.floatLeg.toLocaleString() }}
                  </td>
      ...
      <div
        v-else-if="section === 'IRSB'"
        class="flex flex-col h-full"
      >
        <div class="flex justify-between items-center mb-6">
          <h3 class="text-lg font-bold text-white">
            Treasury Swap Rates (FRED)
          </h3>
          <button
        ...
        <div
          v-if="swapRates.length > 0"
          class="flex-1 bg-black/20 rounded-2xl border border-white/5 overflow-hidden"
        >
          <table class="w-full text-left border-collapse">
            <thead>
      ...
      <!-- ═══ Swap Monitor ═══ -->
      <div
        v-else-if="section === 'IRSM'"
        class="flex flex-col gap-6"
      >
        <h3 class="text-lg font-bold text-white">
          Мониторинг своп-спредов
        </h3>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div
            v-for="ss in swapSpreads"
            :key="ss.name"
            class="p-4 rounded-xl bg-white/5 border border-white/5 text-center"
          >
            <div class="text-xs text-gray-500 uppercase mb-1">
              {{ ss.name }}
            </div>
            <div :class="['text-2xl font-bold font-mono', ss.value >= 0 ? 'text-white' : 'text-rose-400']">
              {{ ss.value.toFixed(1) }}
            </div>
      ...
      <!-- ═══ Asset Swap ═══ -->
      <div
        v-else-if="section === 'ASW'"
        class="grid grid-cols-1 md:grid-cols-2 gap-8 h-full items-center"
      >
        <div class="p-6 rounded-2xl bg-white/5 border border-white/5">
          ...
          <div class="space-y-4">
            <div>
              <label class="text-xs text-gray-500 block mb-1">Bond Price</label><input
                v-model.number="aswBondPrice"
                type="number"
                step="0.01"
                class="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-white font-mono text-sm"
              >
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="text-xs text-gray-500 block mb-1">Coupon (%)</label><input
                  v-model.number="aswCoupon"
                  type="number"
                  step="0.01"
                  class="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-white font-mono text-sm"
                >
              ...
              <div>
                <label class="text-xs text-gray-500 block mb-1">YTM (%)</label><input
                  v-model.number="aswYtm"
                  type="number"
                  step="0.01"
                  class="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-white font-mono text-sm"
                >
        ...
        <div class="p-8 rounded-2xl bg-gradient-to-br from-teal-900/20 to-black border border-teal-500/30 text-center">
          <h3 class="text-sm font-bold text-teal-500 uppercase mb-2">
            Asset Swap Spread
          </h3>
          <div class="text-6xl font-bold text-white mb-2">
            {{ computedAswSpread.toFixed(1) }} <span class="text-2xl text-gray-500">bps</span>
          </div>
