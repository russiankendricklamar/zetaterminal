<template>
  <div class="page-container custom-scrollbar">
    <div class="section-header flex-col gap-4">
      ...
      <!-- Global Stats Bar -->
      <div
        v-if="globalStats && activeTab === 'MARKET'"
        class="flex flex-wrap gap-4 text-xs font-mono"
      >
    ...
    <div v-if="activeTab === 'MARKET'">
      ...
      <div
        v-if="loadError"
        class="p-4 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-400 text-sm font-mono mb-4"
      >
        {{ loadError }}
      </div>

      <div
        class="overflow-auto custom-scrollbar flex-1"
        :class="{ 'opacity-50': loading }"
      >
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="text-xs text-gray-500 border-b border-white/5 uppercase tracking-wider">
              <th class="font-bold py-4 px-4 pl-0 w-8">
                #
              </th>
              <th class="font-bold py-4 px-4">
                Инструмент
              </th>
              <th class="font-bold py-4 px-4 text-right">
                Цена
              </th>
              <th class="font-bold py-4 px-4 text-right">
                1ч
              ...
              <th class="font-bold py-4 px-4 text-right hidden md:table-cell">
                7д
              </th>
              <th class="font-bold py-4 px-4 text-right hidden lg:table-cell">
                Капитализация
              </th>
              <th class="font-bold py-4 px-4 text-right hidden lg:table-cell">
                Объём 24ч
              </th>
              <th class="font-bold py-4 px-4 text-right hidden xl:table-cell w-[120px]">
                7д график
              </th>
            </tr>
          </thead>
          <tbody class="text-sm font-medium">
            ...
            >
              <td class="py-3 px-4 pl-0 text-gray-500 text-xs font-mono">
              ...
              <td class="py-3 px-4">
                <div class="flex items-center gap-3">
                  ...
                  <div
                    v-else
                    class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold bg-gradient-to-br from-orange-500/20 to-yellow-500/20 text-orange-400"
                  >
              ...
              <td class="py-3 px-4 text-right hidden xl:table-cell">
                <svg
                  ...
                  :viewBox="`0 0 120 40`"
                  class="w-[100px] h-[32px] inline-block"
                >
                  <polyline
      ...
      >
        ...
        >
          ...
          <div class="flex items-center justify-between text-xs font-mono">
            ...
            >{{ coin.price_btc.toFixed(8) }} BTC</span>
          </div>
        </div>
      </div>

      <div
        v-if="!trendingLoading && trendingCoins.length === 0"
      ...
      >
        <div class="p-6 rounded-2xl bg-white/5 border border-white/5">
          ...
          <div class="text-3xl font-bold text-white">
            ${{ formatLargeNum(globalStats.total_market_cap?.usd) }}
          </div>
        ...
        <div class="p-6 rounded-2xl bg-white/5 border border-white/5">
          ...
          <div class="text-3xl font-bold text-white">
            ${{ formatLargeNum(globalStats.total_volume?.usd) }}
          </div>
        </div>
        <div class="p-6 rounded-2xl bg-white/5 border border-white/5">
          <h3 class="text-xs font-bold text-gray-400 uppercase mb-2">
            Активных криптовалют
          </h3>
          <div class="text-3xl font-bold text-white">
            {{ globalStats.active_cryptocurrencies?.toLocaleString() }}
          </div>
          <div class="text-xs text-gray-500 mt-1">
            {{ globalStats.markets }} бирж
          </div>
        </div>
      </div>
    ...
    >
      ...
      <div
        v-if="arbOpportunities.length > 0"
        class="overflow-auto rounded-2xl border border-white/5 bg-black/20"
      >
        <table class="w-full text-left border-collapse">
          <thead>
          ...
          <tbody class="text-sm font-mono text-gray-300">
            <tr
              v-for="(opp, idx) in arbOpportunities"
              :key="idx"
            ...
            >
              ...
              <td class="p-3 text-right text-gray-400">
                {{ opp.exchange1 || opp.buy_exchange || '—' }}
              </td>
              <td class="p-3 text-right text-gray-400">
                {{ opp.exchange2 || opp.sell_exchange || '—' }}
              </td>
              <td class="p-3 text-right font-bold text-emerald-400">
                {{ opp.spread || opp.profit_pct || '—' }}
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div
        v-if="!arbLoading && arbOpportunities.length === 0 && !arbError"
        class="flex items-center justify-center h-64 text-gray-500 font-mono text-sm"
      >
