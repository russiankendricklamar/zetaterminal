  ...
  <div class="flex-1 glass-panel rounded-3xl overflow-hidden shadow-2xl shadow-black/20 flex flex-col animate-fade-in h-full">
    ...
    <div class="flex flex-col md:flex-row items-center justify-between p-6 border-b border-white/5 bg-black/20 gap-4">
      <div class="flex items-center gap-4">
        <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-green-600/20 to-emerald-600/20 flex items-center justify-center text-lg font-bold text-green-400 border border-green-500/30">
          <ZapIcon class="w-6 h-6" />
        </div>
        <div>
          <h2 class="text-2xl font-bold text-white tracking-tight">Foreign Exchange</h2>
          <p class="text-xs text-gray-400">Spot Rates, Forwards & Macro Flows</p>
        </div>
      </div>
      ...
      <div class="flex bg-black/40 rounded-xl p-1 border border-white/5 overflow-x-auto max-w-full custom-scrollbar">
        <button 
          ...
          :class="`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-2 whitespace-nowrap ${section === tab.id ? 'bg-green-500/20 text-green-300 border border-green-500/30' : 'text-gray-500 hover:text-white hover:bg-white/5'}`"
        >
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar p-6 bg-gradient-to-b from-black/10 to-transparent">
      ...
      <div v-if="section === 'FXC'" class="flex flex-col h-full">
        <div class="flex justify-between items-center mb-6">
          <h3 class="text-lg font-bold text-white">Cross Rate Matrix</h3>
      ...
      <div v-else-if="section === 'FXCA'" class="grid grid-cols-1 lg:grid-cols-3 gap-8 h-full">
        <div class="p-6 rounded-2xl bg-white/5 border border-white/5 flex flex-col gap-6">
          <h3 class="text-lg font-bold text-white">Converter</h3>
          
          <div class="space-y-4">
            <div>
              <label class="text-xs text-gray-500 font-bold uppercase mb-1 block">Amount</label>
              <input 
                type="number" 
                v-model="amount"
            ...
            <div class="grid grid-cols-[1fr_auto_1fr] gap-2 items-end">
              <div>
                <label class="text-xs text-gray-500 font-bold uppercase mb-1 block">From</label>
              ...
              <div>
                <label class="text-xs text-gray-500 font-bold uppercase mb-1 block">To</label>
          ...
          <div class="mt-auto p-4 rounded-xl bg-black/30 border border-white/10 text-center">
            <div class="text-xs text-gray-500 uppercase mb-1">Result</div>
            ...
            <div class="text-xs text-gray-500 mt-2">Rate: 1 {{ fromCurrency }} = {{ conversionRate }} {{ toCurrency }}</div>
          </div>
        </div>

        <div class="lg:col-span-2 p-6 rounded-2xl bg-white/5 border border-white/5 flex flex-col">
          <div class="flex justify-between items-center mb-6">
            <h3 class="text-lg font-bold text-white">Historical Rate ({{ fromCurrency }}/{{ toCurrency }})</h3>
      ...
      <div v-else-if="section === 'FXFC'" class="flex flex-col h-full gap-6">
        <div class="grid grid-cols-3 gap-6">
          <div class="p-6 rounded-2xl bg-white/5 border border-white/5">
            <h3 class="text-sm font-bold text-gray-400 uppercase mb-2">DXY Forecast (YE)</h3>
            <div class="text-3xl font-bold text-white">102.50</div>
            <div class="text-xs text-rose-400 mt-1">Bearish vs Current (104.20)</div>
          </div>
          <div class="p-6 rounded-2xl bg-white/5 border border-white/5">
            <h3 class="text-sm font-bold text-gray-400 uppercase mb-2">Top Trade (Q4)</h3>
          ...
          <div class="p-6 rounded-2xl bg-white/5 border border-white/5">
            <h3 class="text-sm font-bold text-gray-400 uppercase mb-2">Volatility Outlook</h3>
            <div class="text-3xl font-bold text-white">Rising</div>
        ...
        <div class="flex-1 bg-black/20 rounded-2xl border border-white/5 overflow-hidden">
          <div class="p-4 border-b border-white/10 bg-white/5">
            <h3 class="font-bold text-white">Consensus Forecasts (Median)</h3>
          </div>
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="text-xs text-gray-500 uppercase">
                <th class="p-4">Pair</th>
                <th class="p-4 text-right">Spot</th>
                ...
                <th class="p-4 text-right">Consensus</th>
              </tr>
            </thead>
            <tbody class="text-sm font-mono text-gray-300">
              <tr v-for="(f, i) in forecasts" :key="i" class="border-b border-white/5 hover:bg-white/5 transition-colors">
                ...
                <td class="p-4 text-right">
                  <span :class="`px-2 py-1 rounded text-xs font-bold border ${f.consensus === 'Bullish' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-rose-500/10 border-rose-500/20 text-rose-400'}`">
                    {{ f.consensus }}
                  </span>
                </td>
              </tr>
      ...
      <div v-else-if="section === 'GMM'" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 h-full">
        ...
        <div class="p-6 rounded-2xl bg-white/5 border border-white/5 flex flex-col">
          <h3 class="text-sm font-bold text-gray-400 uppercase mb-4">Macro Indicators</h3>
          <div class="space-y-4 flex-1">
...
const tabs = [
  { id: 'FXC', label: 'Currency Matrix', icon: 'ActivityIcon' },
  { id: 'FXCA', label: 'Conversion', icon: 'ArrowLeftRightIcon' },
  { id: 'FXFC', label: 'FX Forecasts', icon: 'TrendingUpIcon' },
  { id: 'WIRA', label: 'Intl. Reserves', icon: 'DollarSignIcon' },
  { id: 'WCRS', label: 'Rankings', icon: 'BarChart2Icon' },
  { id: 'GMM', label: 'Global Monitor', icon: 'GlobeIcon' },
];
...
const forecasts = [
  { pair: 'EUR/USD', spot: 1.0850, q1: 1.09, q2: 1.10, q3: 1.12, q4: 1.14, consensus: 'Bullish' },
  { pair: 'USD/JPY', spot: 151.20, q1: 150.00, q2: 148.00, q3: 145.00, q4: 142.00, consensus: 'Bearish' },
  { pair: 'GBP/USD', spot: 1.2640, q1: 1.27, q2: 1.28, q3: 1.29, q4: 1.30, consensus: 'Bullish' },
  { pair: 'AUD/USD', spot: 0.6550, q1: 0.66, q2: 0.67, q3: 0.68, q4: 0.70, consensus: 'Bullish' },
  { pair: 'USD/CAD', spot: 1.3580, q1: 1.35, q2: 1.34, q3: 1.33, q4: 1.32, consensus: 'Bearish' },
];
