<template>
  <div class="page-container custom-scrollbar">
      ...
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <p class="section-subtitle font-mono">
            НОВОСТИ, IPO И КОРПОРАТИВНЫЕ СОБЫТИЯ
          </p>
        </div>
      </div>
      <div class="tab-group">
    ...
    >
      <div class="w-3 h-3 border-2 border-purple-500 border-t-transparent rounded-full animate-spin" />
      Загрузка...
    </div>
    ...
    <div class="flex-1 flex flex-col gap-6 p-6">
      ...
      >
        ...
        <div
          v-if="ipoNews.length > 0"
          class="flex-1 space-y-4 overflow-auto"
        >
          <div
            v-for="(article, i) in ipoNews"
            :key="i"
            class="p-4 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 transition-colors cursor-pointer"
          >
            <div class="flex justify-between items-start gap-4">
              ...
              <a
                ...
                class="text-xs text-purple-400 hover:text-purple-300 whitespace-nowrap"
              >Читать</a>
            </div>
          </div>
        </div>
      ...
      >
        ...
        <div class="flex gap-4 items-end">
          ...
          <button
            class="px-6 py-3 bg-purple-600 hover:bg-purple-500 text-white rounded-xl font-bold text-sm transition-colors"
            @click="searchCorpEvents"
          >
            Поиск
          </button>
        </div>
        ...
        <div
          v-if="corpEvents.length > 0"
          class="flex-1 space-y-4 overflow-auto"
        >
          <div
            v-for="(article, i) in corpEvents"
            :key="i"
      ...
      <div
        v-else-if="section === 'QUIC'"
        class="flex flex-col h-full gap-6"
      >
        <h3 class="text-lg font-bold text-white">
          Quick Takes — Анализ настроений
        </h3>
        <div class="flex gap-4 items-end">
          <div class="flex-1">
            <input
        ...
        >
          ...
          <div class="grid grid-cols-3 gap-4">
            <div
              v-for="score in sentimentResult.scores"
              :key="score.label"
              class="text-center p-4 rounded-xl bg-black/20"
            >
      ...
      >
        ...
        <div
          v-if="keyEventNews.length > 0"
          class="flex-1 space-y-4 overflow-auto"
        >
          <div
            v-for="(article, i) in keyEventNews"
            :key="i"
            class="p-4 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 transition-colors"
          >
            <h4 class="text-sm font-bold text-white mb-1">
